...
export interface UIAgent {
  ...
  finishedAt?: string;
  summary?: string;
  /** Optional identity metadata generated by the agent runtime. */
  pokemonName?: string;
  pokemonImage?: string;
}

export interface UIMissionSnapshot {
  task: UITask & { createdAt: string };
  plan: { goal: string; subtasks: UISubtask[] } | null;
  agents: UIAgent[];
  latestLedger: unknown;
  checkpoints: Array<{ from: string; summary: string; artifacts: UIArtifact[] }>;
  nextAction: "review_plan" | "resume_checkpoint" | "continue_work" | "review_results" | "none";
}

export interface UIAgentWorkspaceSnapshot {
  agentId: string;
  files: {
    handsOn: string | null;
    handOff: string | null;
...
export class TaskModel {
  ...
  static async classifyPlanReviewIntent(
    message: string,
  ): Promise<"approve" | "reject" | "feedback"> {
    return this.getElectronAPI().classifyPlanReviewIntent(message);
  }
  ...
  static async continueTask(
    taskId: string,
    message: string,
  ): Promise<{ status: "started" | "conversation" | "task" | "cancelled" | "error"; error?: string }> {
  ...
  static async getRecentTaskEvents(taskId: string): Promise<Array<{ topic: string; data: any }>> {
    return this.getElectronAPI().getRecentTaskEvents(taskId);
  }
  ...
  static async getMissionSnapshot(taskId: string): Promise<UIMissionSnapshot> {
    return this.getElectronAPI().getMissionSnapshot(taskId);
  }

  static async getAgentWorkspace(taskId: string, agentId: string): Promise<UIAgentWorkspaceSnapshot> {
    return this.getElectronAPI().getAgentWorkspace(taskId, agentId);
  }
